...
### C++友元
C++的友元是一种特定的机制，它允许某些外部函数或者类访问另一个类的private或者protected成员。
友元可以是函数、类、或者类成员函数：
1. 友元函数
    一个普通的函数可以被声明为一个类的友元，此时这个函数内部就能获取到类中的private和protected成员了。
    ```cpp
    class MyClass {
    private:
        int x;
...
2. 友元类
    一个类可以被声明为另一个类的友元，此时这个类的所有成员函数就都能访问到另一个类中的private和protected成员了。
    ```cpp
    class MyClass {
    private:
        int x;
...
3. 友元类成员函数
    如果我们不想让一个类中的所有成员函数都能访问到另一个类中的保护和私有成员，我们可以通过声明友元类成员函数来实现。
    ```cpp
    class MyClassB; // 前置声明
    ...
    private:
        int x;
    public:
        MyClassA(int val) : x(val) {}
        friend void MyClassB::display(MyClassA& m);
    };

    class MyClassB {
    public:
        void display(MyClassA& m) {
            std::cout << "MyClassA.x = " << m.x;
        }
    };
    ```

值得注意的是，**友元关系不是相互的**（如果A是B的友元，不代表B是A的友元），**也不是可继承的**（友元关系不会被子类继承）。
...
4. 链接
    ...
    在这个最后一步中，也可以被细分为好几个步骤：
    1. 符号解析
        首先编译器会收集cpp文件生成出来的`.o`或者`.obj`文件中包含的**编译后的机器码和一个符号表**，符号表会列出文件中所有的符号以及它们是否已经被定义了，此时我们就能把符号分成两种：
        * 已定义符号：这种符号在当前的.obj文件中已经被明确定义了，比如在本文件中的函数、全局变量等。
        * 未定义符号：这种符号在当前的.obj文件中被使用了，但是没有明确的定义，可能在其他的.obj文件或者库(lib或者dll)中进行了定义。
        在区分出这两种符号后，**链接器的主要作用就是去别的.obj文件中找到对应的未定义符号的定义**。这个过程会查找到所有的目标文件和库，如果没找到，就会出现**未定义符号**错误；如果找到了，但是出现了多个定义，就会出现**多重定义的错误**。
    2. 地址和偏移分配
        如果链接器找到了所有的符号定义，接下来就是要将.obj中的未定义符号将其定义部分绑定起来，这个过程通常涉及到修改.obj文件中的地址，使其能指向正确的内存空间。
    3. 依赖库处理：
        如果程序使用了外部库，包括静态库和动态库，此时对于静态库，链接器负责将库中的代码合并到最终的可执行文件中。
        而对于动态库，链接器会将其中的符号使用特殊的标记进行记录，然后创建一个**叫做导入表的数据结构**，这个导入表负责告知操作系统在启用这个程序的时候需要帮助我导入哪些dll。同时，由于dll文件每一次启动的位置都是不固定的，因此在编译过程中，**链接器会创建一个叫做存根(Stub)的占位符**，这个占位符会在操作系统启动程序的时候将其替换为对应的dll的实际内存地址。
    4. 优化：在某些情况下，链接器还会进行优化处理，去除掉未使用的代码段，以及合并相似的代码段。
    5. 生成输出文件：通常为exe或者.out文件之类的
...
#### 在多个CPU中进行任务调度的算法(包括多处理器和多核处理器)
TODO：

### 虚拟内存
物理内存有什么问题？
1. 物理内存的空间是有限的
2. 一个进程可以直接通过物理地址访问到另外一个进程，这样不安全，破坏了进程的孤立性
...
**虚拟内存的好处：**
1. 提高了进程的安全性，因为不同的进程之间无法得知其他进程的实际物理地址，同时操作系统也不允许进程直接使用物理地址进行访问。
2. 在某种程度上节省了物理内存的开销。比如说使用fork()函数创造一个子进程的时候，由于两个进程的代码段是相同的，因此两个进程可以通过虚拟内存映射到同一块物理内存的方式进行共享。并且这使得Copy on Write技术得以实现，只有当两个进程的内存出现差别的那一刻，才会复制一份独属于新进程的内存块出来。
3. 可以使进程使用比起物理内存本身大得多的逻辑内存空间，放不下的可以放到硬盘当中。
4. 虚拟内存给予了操作系统更多的关于内存访问的权限控制，比如说操作系统可以通过在PageTable里面增加一些标志位，比如说这个Page是否是只读的，或者是要访问这个Page是否需要Super权限等等。

一个虚拟地址可以分为两个大部分，第一个部分是虚拟页号，用于指向页表(Page Table)中的某一条PTE(entry)，第二部分是一个页内偏移量，这个值是永远都不会变的。
| 第一部分 | 第二部分 |
| ------- | ------- |
| Page Number | Offset |
...
HR面：
1. 如果成功进入实习的话是否有机会转正呢？
2. 后续流程
3. 什么时候出结果呢
4. 我在国外由于时差的关系经常接不到你们那边的电话，后续沟通上要怎么做呢，是通过邮件还是说可以加一个微信？